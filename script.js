...
console.log(isTriangle(1, 1, 2)); // Output: false
...
function highAndLow(numbers) {
  const numberArray = numbers.split(' ').map(Number); // Split the string into an array of numbers

  const highest = Math.max(...numberArray); // Find the highest number using the spread operator
  const lowest = Math.min(...numberArray); // Find the lowest number using the spread operator

  return `${highest} ${lowest}`; // Return the highest and lowest numbers as a string
}
